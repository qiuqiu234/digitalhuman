...
const Chatbot = forwardRef<ChatbotRef, { showChatHistory: boolean }>((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        triggerMic: () => {
            console.log("triggerMic被调用");
            micClick();
            // 5秒后自动关闭
            setTimeout(() => {
                if (isRecording) micClick();
            }, 5000);
        },
    }));
    ...
    const micClick = () => {
        console.log('micClick执行, isRecording:', isRecording, 'isProcessing:', isProcessing);
